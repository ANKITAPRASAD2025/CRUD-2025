...
if (isset($_POST['update'])) {
    ...
    $sql_upload_image = "SELECT * FROM myuser WHERE id={$id} ";
    $query_upload_image = mysqli_query($conn, $sql_upload_image);
    if (!$query_upload_image) {
        echo "no delete images from folder";
    } else {
        echo " delete images from folder";
    }
    $data = mysqli_fetch_assoc($query_upload_image);
    unlink("IMAGES/" . $data['profile_img']);
